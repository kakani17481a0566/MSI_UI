...
export function LeadIdCell({ getValue }) {
  return (
...
export function LeadNameCell({  getValue, column, table }) {
...
export function ContactNumberCell({ getValue, column, table }) {
  const globalQuery = ensureString(table.getState().globalFilter);
  const columnQuery = ensureString(column.getFilterValue());
  const val = getValue();
  ...
  return (
    ...
    <p className="w-48 truncate text-xs-plus xl:w-30 2xl:w-64">
...
export function SchoolNameCell({ getValue, column, table }) {
  const globalQuery = ensureString(table.getState().globalFilter);
  const columnQuery = ensureString(column.getFilterValue());
  const val = getValue();
...
export function LeadSourceNameCell({ getValue, column, table }) {
  const globalQuery = ensureString(table.getState().globalFilter);
  const columnQuery = ensureString(column.getFilterValue());
  const val = getValue();
...
export function LeadTypeNameCell({ getValue, column, table }) {
  const globalQuery = ensureString(table.getState().globalFilter);
  const columnQuery = ensureString(column.getFilterValue());
  const val = getValue();
...
export function LeadStatusCell({ getValue }) {
  const val = getValue();
